...
export const ROLE_PROFILES = {
  "technical-product": {
    ...
    descriptor:
      "Platform Delivery, Product Operations & Cross-Functional Execution",
    positioning:
      "Translates commercial objectives and complex stakeholder requirements into product roadmaps, engineering specifications, UAT plans, and reliable platform launches. Experienced leading technical delivery across engineering, product design, QA, payment integrations, analytics, and external partners.",
    metrics: [
      { value: "82%", label: "Open-beta product adoption" },
      {
        value: "6 months",
        label: "Delivery time eliminated through roadmap alignment",
      },
    ...
    summaryBullets: [
      ...
      { icon: "fas fa-code-branch", text: "REST APIs & Payment Integrations" },
      {
        ...
        text: "PMI-ACP®, FMVA®, Agile Product Owner",
      },
    ],
  },
  ...
  "ai-data-solutions": {
    ...
    descriptor:
      "Agentic Workflows, Analytics Automation & Behavioral Intelligence",
    positioning:
      "Designs applied AI and data systems that turn fragmented operational knowledge into measurable workflows, decision support, and automation. Combines agentic AI, RAG, evaluation harnesses, analytics engineering, and behavioral segmentation with commercial product strategy.",
    metrics: [
  ...
  "it-infrastructure": {
    ...
    descriptor:
      "Enterprise Infrastructure, Service Operations & Technical Team Leadership",
    positioning:
      "Leads business-critical infrastructure across networks, servers, storage, security, support operations, and technical teams. Experienced maintaining reliable environments for offices, commerce systems, publishing operations, and high-volume digital services.",
    metrics: [
    ...
    summaryBullets: [
      { icon: "fas fa-server", text: "Linux, NGINX, Redis & MySQL Stack" },
      {
        ...
        text: "Cisco Networking, VPN & Firewalls",
      },
      { icon: "fas fa-users-cog", text: "Supervised 10+ Technical Staff" },
      {
        icon: "fas fa-certificate",
        text: "IT Infrastructure & Enterprise Storage",
      },
    ],
  },
  ...
  "product-marketing": {
    ...
    descriptor:
      "Positioning, Monetization, Launch Strategy & Strategic Partnerships",
    positioning:
      "Connects product capabilities with differentiated market narratives, monetization systems, launch execution, and lifecycle growth. Experienced leading cross-functional teams, rescuing high-pressure releases, optimizing live operations, and developing partnerships with global technology and entertainment companies.",
    metrics: [
      { value: "+151%", label: "Customer lifetime value" },
      { value: "+237.91%", label: "Month-over-month revenue growth" },
      {
        value: "20 people",
        label: "Cross-functional product and marketing team",
      },
    ],
    skills: [
      "Product Positioning",
    ...
    summaryBullets: [
      { icon: "fas fa-chart-line", text: "GTM Strategy & Product Positioning" },
      {
        ...
        text: "Global Strategic Partnerships (Tencent, AMD)",
      },
      { icon: "fas fa-users", text: "Led 20-person Cross-Functional Team" },
      {
        icon: "fas fa-certificate",
        text: "Monetization & Live-Ops Specialist",
      },
    ],
  },
};
...
function renderRoleProfile(profile) {
  ...
  const positioningEl = document.getElementById(
  ...
  if (metricsGrid && profile.metrics) {
    ...
    profile.metrics.forEach((metric, index) => {
      ...
      card.className =
        "p-3.5 rounded-xl bg-white/60 dark:bg-slate-900/60 border border-slate-200/80 dark:border-slate-800 shadow-sm transition-all";
      ...
      lblDiv.className =
        "text-xs text-slate-600 dark:text-slate-400 font-medium mt-0.5";
  ...
  if (skillsContainer && profile.skills) {
    ...
    profile.skills.forEach((skill) => {
      ...
      chip.className =
        "px-2.5 py-1 rounded-lg bg-indigo-50 dark:bg-indigo-950/70 border border-indigo-200/70 dark:border-indigo-800/70 text-indigo-700 dark:text-sky-300 text-xs font-semibold";
  ...
  if (highlightsContainer && profile.highlights) {
    ...
    profile.highlights.forEach((highlight) => {
      ...
      icon.className =
        "fas fa-check-circle text-indigo-500 dark:text-sky-400 mt-1 text-xs shrink-0";
  ...
  const profileSubtitleEl = document.getElementById(
    "profile-card-role-subtitle",
  );
  ...
  if (profileSubtitleEl)
    profileSubtitleEl.textContent = profile.summarySubtitle;
...
export function initRecruiterView() {
  ...
  function selectRole(roleId, skipAnalytics = false) {
    ...
    roleButtons.forEach((btn) => {
      ...
      if (isTarget) {
        btn.className =
          "recruiter-role-btn h-10 px-4 rounded-xl text-xs font-bold transition-all duration-200 border bg-indigo-600 text-white dark:bg-indigo-600 border-indigo-600 shadow-md shadow-indigo-600/20 flex items-center justify-center gap-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-slate-900";
      } else {
        btn.className =
          "recruiter-role-btn h-10 px-4 rounded-xl text-xs font-semibold transition-all duration-200 border glass-card hover:bg-slate-200/70 dark:hover:bg-slate-800/80 text-slate-700 dark:text-slate-300 border-slate-300/80 dark:border-slate-700 flex items-center justify-center gap-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-slate-900";
      }
    });
