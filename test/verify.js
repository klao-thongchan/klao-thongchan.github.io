...
console.log('[Verification] Starting staging environment check...');
let errors = 0;
...
function verifyHtmlReferences(htmlPath, forbiddenStrings) {
  try {
    const content = fs.readFileSync(htmlPath, 'utf8');
    forbiddenStrings.forEach(str => {
      if (content.includes(str)) {
        console.error(`[Error] Forbidden reference to production asset "${str}" found in ${htmlPath}`);
        errors++;
      }
    });
...
verifyHtmlReferences(INDEX_HTML, ['../css/style.css', '../js/main.js']);
verifyHtmlReferences(CV_INDEX_HTML, ['style.css', 'main.js', 'onclick="window.print()"']);
...
if (errors > 0) {
  ...
  console.log('\n[Verification] SUCCESS! All checks passed, staging environment is isolated and complete.');
  process.exit(0);
}
